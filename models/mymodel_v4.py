...
class MyModelV4(nn.Module):
    ...
    def _decompose_review_by_cf(
        ...
        """
        Decompose review latent with respect to CF latent direction.

        review_latent: [B, D]
        cf_latent:     [B, D]

        shared:
            projection of review_latent onto cf_latent direction

        orthogonal:
            residual orthogonal to cf_latent direction
        """
        cf_basis = F.normalize(
            cf_latent.detach(),
            dim=-1,
            eps=self.orthogonal_eps,
        )

        scale = torch.sum(review_latent * cf_basis, dim=-1, keepdim=True)

        shared = scale * cf_basis
        orthogonal = review_latent - shared

        return shared, orthogonal
    ...
    def forward(
        ...
        review_shared, review_orthogonal = self._decompose_review_by_cf(
