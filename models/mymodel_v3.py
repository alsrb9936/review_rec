...
def _infer_review_input_dim(cfg: DictConfig) -> int:
    data_type = str(cfg.data.get("type", "bert"))
    if data_type.lower() in {"none", "null", ""}:
        data_type = "bert"

    user_review_path = os.path.join(
        str(cfg.data.root),
        str(cfg.data.dataset),
        data_type,
        "user_review_emb.npy",
    )
    if os.path.exists(user_review_path):
        return int(np.load(user_review_path, mmap_mode="r").shape[1])

    if str(cfg.data.get("language_model", "")) == "bert_whitening":
        return int(cfg.data.get("bert_whitening_dim", 64))
    return int(cfg.data.get("plm_embedding_size", 768))
...
class MyModelV3(nn.Module):
    ...
    def __init__(self, cfg: DictConfig, norm_adj: torch.Tensor):
        ...
        self.input_dim = _infer_review_input_dim(cfg)
    ...
    def _dot_score(self, left, right):
        return torch.sum(left * right, dim=-1) / self.review_score_scale
    ...
    def forward(self, user_id, item_id, user_review, item_review, return_dict=False):
        ...
        user_review = user_review.float()
        item_review = item_review.float()
        ...
        if return_dict:
            return {
                ...
                "user_shared_ratio": user_shared_ratio,
                "item_shared_ratio": item_shared_ratio,
                "user_residual_ratio": user_residual_ratio,
                "item_residual_ratio": item_residual_ratio,
            }

        return rating_pred

    def calculate_loss(self, user_id, item_id, user_review, item_review, rating, return_dict=False):
        ...
        loss = rating_loss + self.lambda_side_align * side_align_loss

        if return_dict:
            return {
                "loss": loss,
                "rating_loss": rating_loss.detach(),
                "side_align_loss": side_align_loss.detach(),
                "alpha": outputs["alpha"].mean().detach(),
                "user_shared_ratio": outputs["user_shared_ratio"].mean().detach(),
                "item_shared_ratio": outputs["item_shared_ratio"].mean().detach(),
            }
        return loss
