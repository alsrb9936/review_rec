...
class MyModelV5Dataset(Dataset[dict[str, torch.Tensor]]):
    def __init__(self, cfg: DictConfig, split: str = "train"):
        ...
        if data_type.lower() in {"none", "null", ""}:
            data_type = "bert"
        self.data_dir = os.path.join(str(cfg.data.root), str(cfg.data.dataset), data_type)
        ...
        for path in [user_path, item_path, rating_path]:
            if not os.path.exists(path):
                raise FileNotFoundError(f"Missing MyModelV4 dataset file: {path}")
