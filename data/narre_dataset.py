...
class NARREDataset(Dataset):
    def __init__(self, cfg: DictConfig, split: str = "train"):
        ...
        if self.user_reviews.ndim != 3:
            raise ValueError(f"user_reviews must be 3D [N, review_count, review_length], got {tuple(self.user_reviews.shape)}")
        if self.item_reviews.ndim != 3:
            raise ValueError(f"item_reviews must be 3D [N, review_count, review_length], got {tuple(self.item_reviews.shape)}")
    ...
    def _load_data(self, split: str):
        ...
        return (
            ...
            np.load(rating_path),
            np.load(user_doc_path).astype(np.int64),
            np.load(item_doc_path).astype(np.int64),
